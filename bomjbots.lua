...
if enable_autoupdate then
    ...
    if updater_loaded then
        ...
        if autoupdate_loaded then
            Update.json_url = "https://github.com/Farany1/mscript12414324/blob/main/update.json" .. tostring(os.clock())
            Update.prefix = "[" .. string.upper(thisScript().name) .. "]: "
            Update.url = "https://github.com/Farany1/mscript12414324"
        end
    end
end
...
function kachaccc()
    ...
    while true do
        ...
        sendkey(13)
        wait(7000)
        local result, text, position, distance = GetNearest3DText('Ìàãàçèí')
        if result then
...
function senddatatouser()
    if user == "farany" or "petuh" or "eblan1337" or "opezdal" or "dobriy" or "xack" or "oldi" then
        ...
        elseif user == "oldi" then
            ...
            sampAddChatMessage('ïîø¸ë íàõóé', -1)
        end
        ...
        sampAddChatMessage('Óñïåøíî îòïðàâëåííî', -1)
    else
        sampAddChatMessage('ïîø¸ë íàõóé', -1)
    end
end
...
function main()
    ...
    if autoupdate_loaded and enable_autoupdate and Update then
        ...
        sampAddChatMessage('Ñêðèïòî îáíîâëåí', -1)
    else
        sampAddChatMessage('Îáíîâëåíèå íå òðåáóåòñÿ', -1)
    end
